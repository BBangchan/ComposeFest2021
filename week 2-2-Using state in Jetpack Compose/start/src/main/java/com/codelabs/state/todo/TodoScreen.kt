...
fun TodoRow(
    ...
    iconAlpha: Float = remember(todo.id) { randomTint()} // adding random to a composable
) {
    Row(
        modifier = modifier
        ...
        Icon(
            ...
            contentDescription = stringResource(id = todo.icon.contentDescription) // adding random to a composable
        )
    }
}
...
fun TodoItemInput(
    ...
    Column {
        ...
        if (iconsVisible) { // AnimatedIconRow를 Spacer로 교환하고 있다.
            // navigation Compositive에서처럼 전체 트리를 변경할 수도 있다.
            AnimatedIconRow(
                icon = icon,
